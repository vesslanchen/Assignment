...
For any distinct indices $i,j$, the transition map $\phi_j^\pm\circ (\phi_i^\pm)^{-1}$ is easily computed. In the case $i<j$, we get 
\begin{align}
    \phi_i^\pm\circ (\phi_j^\pm)^{-1}(u^1,\cdots,u^{n+1})=(u^1,\cdots,\hat{u}^{i},\cdots,\pm\sqrt{1-|u|^2},\cdots,u^{n})
\end{align}
