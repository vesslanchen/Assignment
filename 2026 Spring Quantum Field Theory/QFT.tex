\documentclass[12pt,letterpaper]{article}

\usepackage[letterpaper, left=1cm, right=1cm, top=1.5cm, bottom=1.5cm]{geometry}
\usepackage[compat=1.0.0]{tikz-feynman}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{amssymb}
\usepackage{graphicx}
\usepackage{setspace} % 引入這個套件
\usepackage{tensor}
\usepackage{braket}
\usepackage{slashed}
\usepackage{subcaption}
\usepackage{cancel}
\usepackage{hyperref}\hypersetup{colorlinks=true,linkcolor=blue,citecolor=green,urlcolor=red}

\title{University of Minnesota\\
School of Physics and Astronomy\\
\textbf{2026 Spring Physics 8012\\ Quantum Field Theory II}\\ 
Assignment Solution}
\author{Lecture Instructor: Professor Mikhail "Misha" Shifman \\ \\
        Zong-En Chen\\ chen9613@umn.edu}
\date{\today}  

% 自定義範例環境
\newtheorem{example}{Example}[section]
\newtheorem{solution}{Sol.}[section]
% 自定義習題環境
\newtheorem{exercise}{Ex.}[section]

\newtheorem{theorem}{Theorem}[section]

\newcommand{\question}[2]{\section*{Question #1} #2}
\newcommand{\answer}[1]{\section*{Answer}#1}

\begin{document}
\onehalfspacing
%\maketitle
\begin{titlepage}
    \centering
    \vspace*{\fill} % 從頂端往下推到中間
    {\LARGE University of Minnesota\\
    School of Physics and Astronomy\\[1em]
    \textbf{2026 Spring Physics 8012\\ Quantum Field Theory II}\\ 
    Assignment Solution \par}
    \vspace{2cm}
    {\large Lecture Instructor: Professor Mikhail "Misha" Shifman \par}
    \vspace{1cm}
    {\large Zong-En Chen\\ chen9613@umn.edu \par}
    \vspace{2cm}
    {\large \today \par}
    \vspace*{\fill} % 從底部往上推到中間
\end{titlepage}

<<<<<<< HEAD
\include{Homework 1/HW1}
\include{Homework 2/HW2}
\include{Homework 3/HW3}
\include{Homework 4/HW4}
\include{Homework 5/HW5}
\include{Homework 6/HW6}
\include{Homework 7/HW7}
\include{Homework 8/HW8}
\include{Homework 9/HW9}
\include{Homework 10/HW10}
\include{Homework 11/HW11}
=======
%\include{Homework 1/HW1}
%\include{Homework 2/HW2}
%\include{Homework 3/HW3}
%\include{Homework 4/HW4}
%\include{Homework 5/HW5}
%\include{Homework 6/HW6}
%\include{Homework 7/HW7}
%\include{Homework 8/HW8}
%\include{Homework 9/HW9}
%\include{Homework 10/HW10}
%\include{Homework 11/HW11}
>>>>>>> 419d8652be3570af19463f70b3ac31339d540a28
\include{Homework 12/HW12}
\end{document}